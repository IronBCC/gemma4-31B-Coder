...
- **Next**: (16) verify v7 manifests+audit (Codex building: per-source counts read, pipeline scripts read) → 1-step smoke → 5-step from cp20 @2e-5 (--sm120-attn) → hard30 vs same-day cp20 (≥+3) → 30-case SWE-Lite at **temperature 0.7** (standing gate config) ≥18/30 non-empty. Launch command = the Gate-B-verified nohup with `--data ..._v7_budget49152`.
